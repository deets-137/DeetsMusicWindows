...
| Sound panel | Equalizer · Adaptive tabs | The equalizer: the curve, the sliders and the presets · Adaptive sound: matched loudness, fuller low volume and headphone crossfeed | sound-panel.ts |
...
| Sound panel | Equalizer · Adaptive sound pills | Turns the equalizer on or off · Turns adaptive sound on or off: matched loudness, fuller low volume and headphone crossfeed | sound-panel.ts |
...
| Sound panel | Preamp · ‹ › · Per output · × | How a boost is kept from clipping: the limiter alone, lower the song only when the volume leaves no room, always lower it by the boost, or a level you set · 0.5 dB lower / higher · On: each output remembers its own preset, and switching output switches the preset · Forgets the preset for this output | sound-panel.ts |
| Sound panel | Match loudness · Fuller at low volume · Headphone crossfeed | Plays each song at the same loudness · Adds bass and a little treble as the volume goes down, the way the ear loses them · Mixes a little of each channel into the other on headphones. Auto: only when Windows reports headphones | sound-panel.ts |
| Sound panel | Loudness target · Album gain · New songs · Low volume follows · Crossfeed amount | How loud every song is made: −16 is Apple's Sound Check level, −14 is louder, −18 is quieter · On: an album played in order keeps one gain, so its quiet songs stay quiet · A song not measured yet: the library's median gain, or no change · Which volume Fuller at low volume follows · How much of each channel goes into the other | sound-panel.ts |
| Sound panel | footer: Keep: 7 days pill · Keep · Turn all off · the meter | *Asks whether to keep the effects on Sep 23* (or: this long after you first turn one on · never · you chose to keep them). Click for the next choice · Keeps the effects and stops asking · Turns the equalizer and adaptive sound off · The limiter's deepest cut and the loudest sample in the last quarter second | sound-panel.ts |
