...
> queue model this bends), [DATA-ARCHITECTURE.md](../architecture/DATA-ARCHITECTURE.md) (provider trait + cache),
> [SURFACES-AND-CARDS.md](../architecture/SURFACES-AND-CARDS.md) (the card system).
...
([TOASTS.md](../architecture/TOASTS.md) §5). Menu position: after Add to Playlist, before Add to Library.
...
([UI-ARCHITECTURE §4a](../architecture/UI-ARCHITECTURE.md)) with one root grouping over a **heterogeneous shelf
...
(Rust `MusicProvider` trait, [DATA-ARCHITECTURE](../architecture/DATA-ARCHITECTURE.md)) — the same seam the
...
[Song of the Day](../integrations/DeetsOTD.md) (built since 0.11.0; curated) — feed the *same* weighting; thumbs is the **in-station
