...
> Apple. Siblings: [HANDOFF](../HANDOFF.md) · [DATA-ARCHITECTURE](../architecture/DATA-ARCHITECTURE.md)
> (model / provider / cache) · [UI-ARCHITECTURE](../architecture/UI-ARCHITECTURE.md) (collection-card
> engine) · [SURFACES-AND-CARDS](../architecture/SURFACES-AND-CARDS.md) (the card system this rides) ·
...
- **`＋ New Playlist`** → name input → empty local playlist → drills in → the empty state.
  **The empty pane is the filled pane's skeleton (2026-09-15).** The hero holds the full width,
  ...
  ([UI-ARCHITECTURE.md](../architecture/UI-ARCHITECTURE.md) §4a, *An empty pane keeps the shape*).
