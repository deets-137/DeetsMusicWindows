...
| Library album or artist level (2026-09-24, FULL-LIB.md) | Full · Lib chips | Shows the whole album or artist from Apple Music · Shows only the songs in your library · when Lib cannot be picked: *None of these songs are in your library* (album) / *No songs by this artist are in your library* (artist) / *Loading…* | collection-card.ts (`paintViews`), library-full.ts |
| Library Full view rows (2026-09-24) | the Add-to-Library square | Add to Library · *In your library* (the ✓ shows at rest here, whatever Show ✓ on songs you have says) · an unreleased row: the release hint, as in Search | add-square.ts (`mark`) |
