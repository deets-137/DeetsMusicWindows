...
- Adding **Spotify** later = a new provider impl + adapter in Rust; **zero UI change** for the
  ...
  direct Apple commands need) is [PROVIDERS.md](../integrations/PROVIDERS.md), designed 2026-09-15.
...
> build cannot carry one. See [RELEASE.md](../ops/RELEASE.md) §7 for the Cloudflare Worker that
...
mirror + local store in `playlists.rs` ([PLAYLISTS.md](../features/PLAYLISTS.md)); local rows ride a
...
read-in, [PLAYLISTS.md](../features/PLAYLISTS.md)) and the Search/enrichment catalog paths; it will
...
**Add times** (2026-09-15, [HOME.md §3](../features/HOME.md)):
```sql
...
| WAL files | `<app_data_dir>/deetsmusic.db-wal`, `-shm` (WAL mode since 2026-09-16, [LOCAL-DATA.md](../integrations/LOCAL-DATA.md) §3) | n/a (runtime) |
...
| Shipped installers | `installers/` | ✗ gitignored ([RELEASE.md](../ops/RELEASE.md)) |
| Last.fm session | `<app_data_dir>/lastfm-session.json` | n/a (runtime) — `{name, key}`; the key never reaches the renderer or the log, [LASTFM.md](../integrations/LASTFM.md) |
| Last.fm API key + secret | `Documents\Deets' Secrets\lastfm.json`, built into the exe by `build.rs` | ✗ outside the repo |
| Cached developer token | `<app_data_dir>/developer-token.json` | n/a (runtime) — `{token, exp, source, config}`; written only when the mint is used, [RELEASE.md](../ops/RELEASE.md) §7 |
...
be put back — [TRAY.md](../features/TRAY.md) §5). Every field has a default, so a missing or stale file
