...
| `sotd.ts` `withdrawAsking` (2026-09-18) | warn, sticky | Take the Discord post for *"Song"* down? Your pick stays. **[Withdraw] [Cancel]** | [DeetsOTD.md](../integrations/DeetsOTD.md) §10.6. Asks because a delete cannot be undone: the same message can never be reposted. On success, a `success` toast — "Withdrawn from Discord. The pick is still yours." — and a `warn` if the outlet refused. |
| `sotd.ts` `unmarkAsking` · a replaced pick (2026-09-18) | info, sticky / warn, sticky | Also delete the Discord post? **[Delete] [Keep]** / Also delete the posts of the pick you replaced? **[Delete] [Cancel]** | 9d: Delete is the first button. Only shown when a post really went out. The unmark ask keeps **Keep**: it is not a decline, it unmarks and keeps the post. The replaced-pick ask is a plain destructive question (warn, Cancel, 2026-09-25). |
...
| `library-add.ts` — one of Apple's playlists (2026-09-24) | warn / success | Couldn't add “Name” to your library. / Playlist added to your library. | The playlist menus' Add to Library (CONTEXT-MENUS.md §3.4). The first add ever raises the once-notice below with "Playlist added." in place of "Added." |
...
| `apple-health.ts` | warn | DeetsMusic can't reach Apple Music. Check your internet connection. **[Try again]** | app=`unreachable`. Same 5-min recheck. Row: "Connected · Offline". **Resume (2026-09-14):** when a playback error found this cause (a network drop kills MusicKit's audio player with `loadSegmentError`), the song and last heard position are held; the next check that finds Apple healthy reloads that song and seeks back — once, queue mode only, skipped if anything played or the song changed (`player:resumeArmed` / `resumeAfterReconnect` / `resumeSkip` in the log). **No MusicKit (2026-09-25):** a launch with no network fails the MusicKit `<script>` in index.html, and a play then waited forever with no word. Now a play click while it is missing shows this toast (forced) and does not replay the click (his call). `player.ts` loads the script again on the `online` event, every 30 s and on each such click (`player:musicKitRetry` in the log). |
| `apple-health.ts` — Apple said "too many requests" to a user call | warn (6 s) | Apple Music is busy right now. Try again in {N} seconds. / … Try again in a minute. | Added 2026-09-25 (APPLE-CALLS.md §3, his words). Rust's `apple-busy` event, sent only for a user call (a background job skips quietly). N is `Retry-After`; "a minute" when Apple sent none. A second 429 within 6 s adds no toast. |
...
| `diary-card.ts` `askDelete` — Delete Entry (2026-09-24) | warn, sticky **question** | *Delete your Diary entry for “OPIA”? Its notes and scores go with it.* **[Delete] [Cancel]** | No Undo, so it asks first. DIARY.md §6 |
| `diary-card.ts` `deleteFolder` — Delete Folder (2026-09-25) | info | *Deleted the folder “F”. Its entries stay.* **[Undo]** | The destructive-action rule: a folder delete can be undone, so it runs at once. Undo remakes the folder under its old name and files its entries back (a new id). A failed Undo shows the Diary's save warn. |
...
| `favorites.ts` `setAlbumLoved` / `setPlaylistLoved` — the Apple write fails (2026-09-24) | warn | Couldn't update Favorites for “Name”. | An album or playlist ♥ (CONTEXT-MENUS.md §3b); flipped back first, as a song's. |
| `favorites.ts` `setAlbumLoved` — a Library album with no Apple id | warn | Couldn't find “Name” on Apple Music. | The song → album hop found no album (an upload, a withdrawn album). |
...
| `playlists-card.ts` `undoToast` — Remove from Playlist, Remove N picked, Delete Folder, Remove Cover, Generate Cover › Mosaic (2026-09-25) | info | Removed “Song” from “X”. / Removed N songs from “X”. / Deleted the folder “F”. Its playlists stay. / Removed the cover of “X”. / “X” shows the mosaic again. **[Undo]** — and *Couldn't undo that.* (warn) when the Undo fails | The destructive-action rule (his call, 2026-09-25): what can be undone runs at once and offers Undo; what cannot asks first, with Cancel. Undo puts the songs back at their old positions, remakes the folder under its old name with its playlists (a new id: a moved folder row goes back to the default order), and saves the old cover again (read back from its cover link before the write). Delete Playlist still asks: a restore is a new id without its Apple link, pins or place. |
...
| `airplay.ts` — a dropped speaker came back | info | Reconnected to {speaker}. | Added 2026-09-25 (AIRPLAY.md §13.3). The speaker dropped the session (a PC sleep, a Wi-Fi drop) and a reconnect try worked; the music plays again if the drop paused it. |
| `airplay.ts` — a dropped speaker did not come back | error (sticky) **[Try again]** | Lost {speaker}. The music is paused. / Lost {speaker}. | Added 2026-09-25 (AIRPLAY.md §13.3). All 3 reconnect tries (about 30 s) failed. Sticky: it can come while nobody watches. The second text when nothing was playing. Try again connects, then plays if the drop paused the music. |
...
| `room-friends.ts` — a room member asks to be friends | info **(sticky, with actions)** | {Name} wants to add you as a friend. **[Add] [Not now]** | Added 2026-09-26 (FRIENDS.md §18, F2A). An ask, so it takes the queue's priority. Closed by the room ending, or by Add friend on their row. Not now tells them nothing (F4A). |
| `room-friends.ts` — the mutual add is done | info | {Name} is now your friend. / {Name} is already your friend. | Added 2026-09-26. Both sides, once each. Replaces `addFriend`'s "once they add you back", which is wrong here. |
| `room-friends.ts` — the ask did not go | info / warn | This room can't carry a friend ask yet. Add them by code in Friends. / {Name} has left the room. / Couldn't send your friend code. (warn) | Added 2026-09-26. The first is a rooms worker from before the relay. |
| `room-friends.ts` — no friend code could be made | warn | Couldn't make your friend code. | Added 2026-09-26. |
...
- **A destructive action** (his call, 2026-09-25): if it can be undone, it runs at once and
  its `info` toast offers **[Undo]**. If it cannot, it asks first in a sticky question whose
  decline button reads **Cancel**. "Can be undone" means the Undo gives back the same thing,
  not a copy with a new id that loses links, pins or place. An offer (Join / Not now) and a
  two-way choice whose second button acts (Delete / Keep the post) are not declines.
