...
| **Split pill** (two answers in a head, 2026-09-24) | A card head: "Full \| Lib" in the Library (FULL-LIB.md) and the Search card (SEARCH-FIELDS.md §6) | The icon square's family as a worded action (`.panel__action--text`) cut in two as the room pill is: `--icon-lg` tall, `--surface-hover` fill, `1px --border`, `--split-pill-radius`, equal halves with a 1 px `--border` divider, `--title` words, the pressed half `--picked`, a half that cannot be picked at `--split-pill-dim`. One primitive: `split-pill.ts` | `.split-pill` |
...
ends upright at any of the three speeds, and a song change slides the discs as Web Animations — [VINYL.md](../features/VINYL.md), with its
...
Add to Playlist ▸ — see [DATA-ARCHITECTURE](DATA-ARCHITECTURE.md) / [QUEUE.md](../features/QUEUE.md) /
[PLAYLISTS.md](../features/PLAYLISTS.md) for the action sides); Artists declare no `menu`, so they
...
**Artist views (2026-09-15, [ARTIST-VIEW.md](../features/ARTIST-VIEW.md))** also open on a hero: a round
...
MusicKit gaplessly (see [QUEUE.md](../features/QUEUE.md)). Tokens: `--drag-lift`, `--drop-line-w` (skin);
