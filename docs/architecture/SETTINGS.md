...
| Skin settings | Canvas (Glass only. What shows behind the cards: the theme's glow, the album covers, or a picture you choose) — pills (2026-09-24); shows only while Glass is the skin | `glassCanvas` | **aurora** / covers / picture | `wallpaper.ts` fills the `.wallpaper` layer and sets `data-wallpaper` on `<html>` while it shows. COVER-WALLPAPER.md |
| Skin settings | Tiles (Glass only. How many album covers sit around the one that plays. One cover: it fills the window alone) — a menu of four (2026-09-24); shows with Canvas on Covers | `glassTiles` | one / few / **some** / many | `wallpaper.ts` `TILE_COUNT` (0 / 6 / 12 / 20). *One cover* is layout 1A (the cover alone); the owner kept it as a choice at the desk test, 2026-09-24, with the mosaic as the default |
| Skin settings | Picture (Glass only. The picture behind the cards. Press Choose, or drop an image file on this row) — an action (2026-09-24); shows with Canvas on Picture | `glassPicture` | **0** (none) / a stamp | The stamp of the last choice, ms. The file is `wallpaper.jpg` in the app data folder (wallpaper.rs), shown at `http://wallpaper.localhost/<stamp>` |
| Skin settings | Diffusion (Glass only. Softens the picture behind the cards. 0: sharp) — slider 0–100% (2026-09-24); shows with Canvas on Covers or Picture; never locked by Fancy Glass | `glassDiffusion` | **30** / 0–100 | `skin-settings.ts` → `--glass-diffusion` on `<html>` → `.wallpaper__set` `filter: blur()` at that share of `--wallpaper-diffusion-max` (20 px) |
| Skin settings | Aurora color (Glass only. Cover: the glow takes its colors from the picture behind the cards. Theme: the theme's own colors) — pills (2026-09-24); shows with Canvas on Covers or Picture | `glassAuroraColor` | **cover** / theme | `wallpaper.ts` sets `--canvas-go` / `--canvas-stop` / `--canvas-pause` on `<html>` (the album palette through `auroraSlots`, or the picture's own colors) |
...
| Diary | Grow on open (The Diary grows over the card beside it when an album opens: taller in Max, wider in Midi) — *New entries* / *Every entry* / *Never* | `diaryGrow` | **new** / every / never | `diary-card.ts` `showEntry` → `card-grow.ts` `growCardTaller` |
...
| Connections | Agents use the Diary (Lets a connected agent read your Diary and write notes, scores and dates in it) — 2026-09-24 | Rust `agentDiary` (`settings_set_agent_diary`) | on / **off** (agents: off only) | `bridge.rs` `/diary` — [DIARY.md §10](../features/DIARY.md) |
...
(Glass: Canvas and the rows it shows — Tiles, Picture, Diffusion, Aurora color — then Fancy Glass, which shows the rest, then Canvas glow, Dim canvas, Backlight, Tint cards; Press: Record player, then Show record
